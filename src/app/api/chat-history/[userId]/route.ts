...
export async function GET(request: NextRequest, { params }: { params: { userId: string } }) {
  try {
    ...
    const chatHistory = await ChatHistory.find({ userId: new ObjectId(userId) });
...
export async function POST(request: NextRequest, { params }: { params: { userId: string } }) {
  try {
    ...
    const { userId } = params;
    const { messages, chatId }: { messages: { content: string; sender: 'user' | 'bot' }[], chatId?: string } = await request.json();

    // Validate ObjectId format
    if (!ObjectId.isValid(userId) || (chatId && !ObjectId.isValid(chatId))) {
      return NextResponse.json({ message: 'Invalid user ID or chat ID format' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: 'Error saving messages', details: error.message }, { status: 500 });
  }
}